...
<table class="table"><thead><tr><th>Ngày sử dụng</th><th>Booking</th><th>Lượt sân</th><th>Lượt đã hủy</th></tr></thead><tbody>@forelse($bookings['daily']->sortKeys() as $date=>$row)<tr><td>{{ $date }}</td><td>{{ $row['bookings'] }}</td><td>{{ $row['slots'] }}</td><td>{{ $row['cancelled'] }}</td></tr>@empty<tr><td colspan="4">Chưa có dữ liệu.</td></tr>@endforelse</tbody></table>
</section>
...
<div class="row"><div class="col-md-6"><h3 class="h5">Nguyên nhân</h3><table class="table"><thead><tr><th>Lý do</th><th>Giao dịch</th><th>Tiền đã hoàn</th></tr></thead><tbody>@forelse($refunds['reasons'] as $reason=>$row)<tr><td>{{ \App\Models\RefundRequest::REASONS[$reason] ?? 'Khác / chưa phân loại' }}</td><td>{{ $row['count'] }}</td><td>{{ number_format($row['amount']) }}đ</td></tr>@empty<tr><td colspan="3">Chưa có khoản hoàn thành công.</td></tr>@endforelse</tbody></table></div>
<div class="col-md-6"><h3 class="h5">Sân liên quan</h3><table class="table"><thead><tr><th>Sân</th><th>Giao dịch</th><th>Tiền đã hoàn</th></tr></thead><tbody>@forelse($refunds['courts'] as $court=>$row)<tr><td>{{ $court }}</td><td>{{ $row['count'] }}</td><td>{{ number_format($row['amount']) }}đ</td></tr>@empty<tr><td colspan="3">Chưa có khoản hoàn thành công.</td></tr>@endforelse</tbody></table></div></div>
