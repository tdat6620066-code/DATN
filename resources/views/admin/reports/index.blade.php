...
<div class="table-responsive"><table class="table"><thead><tr><th>Ngày thanh toán</th><th>Doanh thu sân</th><th>Phương thức thu</th></tr></thead><tbody>@forelse($revenue['daily']->sortKeysDesc() as $date=>$amount)<tr><td>{{ $date }}</td><td>{{ number_format($amount) }}đ</td><td>@include('admin.reports.method-breakdown', ['methods' => $revenue['daily_methods'][$date] ?? collect()])</td></tr>@empty<tr><td colspan="3"><x-admin.empty message="Chưa có thanh toán thành công trong kỳ." /></td></tr>@endforelse</tbody></table></div>
<h2 class="h5">Theo sân</h2><div class="table-responsive"><table class="table"><thead><tr><th>Sân</th><th>Lượt thuộc giao dịch đã thanh toán</th><th>Doanh thu</th><th>Phương thức thu</th></tr></thead><tbody>@foreach($revenue['courts'] as $row)<tr><td>{{ $row['name'] }}</td><td>{{ $row['slots'] }}</td><td>{{ number_format($row['amount']) }}đ</td><td>@include('admin.reports.method-breakdown', ['methods' => $row['methods']])</td></tr>@endforeach</tbody></table></div>
</section>
...
<div class="table-responsive"><table class="table"><thead><tr><th>Ngày thanh toán</th><th>Booking</th><th>Lượt sân</th><th>Lượt đã hủy</th></tr></thead><tbody>@forelse($bookings['daily']->sortKeysDesc() as $date=>$row)<tr><td>{{ $date }}</td><td>{{ $row['bookings'] }}</td><td>{{ $row['slots'] }}</td><td>{{ $row['cancelled'] }}</td></tr>@empty<tr><td colspan="4"><x-admin.empty message="Chưa có dữ liệu." /></td></tr>@endforelse</tbody></table></div>
</section>
...
<div class="row"><div class="col-md-6"><h3 class="h5">Nguyên nhân</h3><div class="table-responsive"><table class="table"><thead><tr><th>Lý do</th><th>Giao dịch</th><th>Tiền đã hoàn</th><th>Phương thức hoàn</th></tr></thead><tbody>@forelse($refunds['reasons'] as $reason=>$row)<tr><td>{{ \App\Models\RefundRequest::REASONS[$reason] ?? 'Khác / chưa phân loại' }}</td><td>{{ $row['count'] }}</td><td>{{ number_format($row['amount']) }}đ</td><td>@include('admin.reports.method-breakdown', ['methods' => $row['methods']])</td></tr>@empty<tr><td colspan="4"><x-admin.empty message="Chưa có khoản hoàn thành công." /></td></tr>@endforelse</tbody></table></div></div>
<div class="col-md-6"><h3 class="h5">Sân liên quan</h3><div class="table-responsive"><table class="table"><thead><tr><th>Sân</th><th>Giao dịch</th><th>Tiền đã hoàn</th><th>Phương thức hoàn</th></tr></thead><tbody>@forelse($refunds['courts'] as $court=>$row)<tr><td>{{ $court }}</td><td>{{ $row['count'] }}</td><td>{{ number_format($row['amount']) }}đ</td><td>@include('admin.reports.method-breakdown', ['methods' => $row['methods']])</td></tr>@empty<tr><td colspan="4"><x-admin.empty message="Chưa có khoản hoàn thành công." /></td></tr>@endforelse</tbody></table></div></div></div>
