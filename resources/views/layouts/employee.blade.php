...
<div class="staff-shell">
    <aside class="staff-sidebar" id="staffSidebar">
        ...
        <nav class="staff-nav"><a href="{{ route('incident-tickets.index') }}">Sự cố & Khiếu nại [{{ \App\Models\CourtIncident::where('source','CUSTOMER')->where('status','PENDING')->count() }}]</a></nav><div class="staff-section-label">SỰ CỐ</div><nav class="staff-nav"><a class="{{ request()->routeIs('employee.incidents.*') ? 'active' : '' }}" href="{{ route('employee.incidents.index') }}"><i class="bi bi-exclamation-triangle"></i>Báo cáo sự cố</a></nav>
        @endif
    ...
    <div class="staff-content">
        <header class="staff-topbar">
            ...
            <div class="staff-topbar-actions"><a href="{{ route('home') }}" title="Xem trang khách hàng"><i class="bi bi-box-arrow-up-right"></i></a><a href="{{ route('notifications.index') }}" title="Thông báo"><i class="bi bi-bell"></i> {{ \App\Models\Notification::where('user_id',auth()->id())->where('is_read',false)->count() }}</a></div>
        </header>
