...
<div class="admin-top-actions"><a class="btn btn-outline-success btn-sm text-nowrap" href="{{ route('admin.dashboard') }}"><i class="bi bi-speedometer2 me-1" aria-hidden="true"></i>Dashboard</a>@include('partials.notification-dropdown')<a class="admin-profile" href="{{ route('admin.users.edit', auth()->user()) }}"><span>{{ Str::upper(Str::substr(auth()->user()->name,0,1)) }}</span><strong>{{ auth()->user()->name }}</strong></a></div>
...
@if(request()->routeIs('admin.dashboard', 'admin.reports.index', 'admin.reports.cash-flow'))
<script defer src="{{ asset('js/live-reports.js') }}?v={{ filemtime(public_path('js/live-reports.js')) }}"></script>
@endif
</body></html>
