...
class AdminEmployeeTest extends TestCase
{
    ...
    public function test_admin_can_create_employee_with_permissions(): void
    {
        ...
        $this->actingAs($admin)->post(route('admin.employees.store'), ['name' => 'Nhân viên mới', 'email' => 'staff@example.com', 'password' => 'password123', 'password_confirmation' => 'password123', 'role' => 'EMPLOYEE', 'permissions' => ['employee.dashboard']])->assertRedirect(route('admin.employees.index'));
        $staff = User::where('email', 'staff@example.com')->firstOrFail();
        $this->assertSame(['employee.dashboard'], $staff->permissions);
    }
