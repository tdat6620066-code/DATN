...
class AdminCourtController extends Controller
{
    ...
    {
        return $request->validate(['name' => ['required', 'string', 'max:255', Rule::unique('courts', 'name')->ignore($court)], 'court_type_id' => ['required', 'exists:court_types,id'], 'description' => ['nullable', 'string', 'max:5000'], 'address' => ['nullable', 'string', 'max:500'], 'phone' => ['nullable', 'string', 'max:30'], 'opening_time' => ['required', 'date_format:H:i'], 'closing_time' => ['required', 'date_format:H:i', 'after:opening_time'], 'status' => ['required', Rule::in(['ACTIVE', 'INACTIVE'])], 'is_featured' => ['nullable', 'boolean'], 'amenity_ids' => ['nullable', 'array'], 'amenity_ids.*' => ['integer', 'exists:amenities,id'], 'prices' => ['required', 'array'], 'prices.*' => ['nullable', 'numeric', 'min:0', 'max:999999999'], 'images' => ['nullable', 'array', 'max:8'], 'images.*' => ['image', 'mimes:jpg,jpeg,png,webp', 'max:5120'], 'remove_image_ids' => ['nullable', 'array'], 'remove_image_ids.*' => ['integer']]);
    }
