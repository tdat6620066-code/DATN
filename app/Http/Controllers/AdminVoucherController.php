...
class AdminVoucherController extends Controller
{
    ...
    {
        $this->admin($request);
        $vouchers = Voucher::when($request->filled('search'), fn ($q) => $q->where(fn ($search) => $search->where('code', 'like', '%'.$request->search.'%')->orWhere('name', 'like', '%'.$request->search.'%')))->when($request->filled('status'), fn ($q) => $q->where('status', $request->status))->latest()->orderByDesc('id')->paginate(15)->withQueryString();
