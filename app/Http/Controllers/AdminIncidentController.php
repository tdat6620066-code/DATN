...
{
    public function index(Request $request) { $incidents=CourtIncident::where('source','COURT')->with(['court','reporter'])->when($request->status,fn($q,$s)=>$q->where('status',$s))->latest()->paginate(20)->withQueryString(); return view('admin.incidents.index',compact('incidents')); }
